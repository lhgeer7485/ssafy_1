...
def is_validation(user_dict):
    ...
    #a
    result_a = False
    for a in range(len(blood_types)):
        if user_dict['blood_group'] == blood_types[a]:
            result_list[0] = True
            result_a = True
            break
    if result_a == False:
    ...
    for b in range(len(black_list)):
        if user_dict['company'] == black_list[b]:
            result_list[0] = 'blocked'
            #print(result_list)
            return result_list
    result_b = True
    if result_b == True:  
...
def create_user(user_dict):
    global new_data
    result = is_validation(user_dict)
    global count
    ...
    if result[0] == False or result[0] == 'blocked':
        count += 1

    if result[0] == False:
        for j in range(len(result[1])):    
            if result[1][j] == 'blood_group':
                user_dict['blood_group'] = None
            if result[1][j] == 'mail':
                user_dict['mail'] = None
            if result[1][j] == 'name':
                user_dict['name'] = None
            if result[1][j] == 'website':
                user_dict['website'] = None
