...
SELECT Name, Milliseconds / 60000  AS '재생 시간' FROM tracks;
-- 02. Sorting data
...
SELECT Name, Milliseconds / 60000 FROM tracks ORDER BY Milliseconds ASC;
-- NULL 정렬 예시
...
SELECT Name, Bytes FROM tracks WHERE Bytes BETWEEN 100000 AND 500000 ORDER BY Bytes ASC;
...
SELECT TrackId, Name, Bytes FROM tracks ORDER BY Bytes DESC LIMIT 3, 4; -- 4~7까지
-- 04. Grouping data
SELECT Country, COUNT(*) FROM customers GROUP BY Country;
SELECT Composer, AVG(Bytes) FROM tracks GROUP BY Composer ORDER BY AVG(Bytes) DESC;
SELECT Composer, AVG(Bytes) AS '평균' FROM tracks GROUP BY Composer ORDER BY 평균 DESC;
SELECT AVG(Milliseconds / 60000) AS 'AVG_M' FROM tracks GROUP BY Composer HAVING AVG_M < 10;
